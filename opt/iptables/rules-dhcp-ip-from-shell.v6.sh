...
if [ "$RUN_SILENTLY" != "1" ]; then
	echo ""
 	logger "$(basename "$0")(IPv6): Connection tracking modules loaded (if enabled)"
fi
...
# similar to the IPv4 method when the Don't Fragment bit is set. Therefore, the following
# is not necessary:
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type router-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type router-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type router-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type router-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type 130 -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type 130 -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type 131 -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type 131 -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_llcl_icmp_in -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_llcl_icmp_out -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type 143 -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type echo-request -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type echo-request -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A ext_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type echo-request -j ACCEPT
$IP6TABLES -A ext_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type echo-reply -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type router-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_llcl_icmp_in -p icmpv6 --icmpv6-type router-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type router-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_llcl_icmp_in -p icmpv6 --icmpv6-type router-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type router-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_llcl_icmp_out -p icmpv6 --icmpv6-type router-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type 130 -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type 130 -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type 131 -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type 131 -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type 132 -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type 132 -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_llcl_icmp_in -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_llcl_icmp_out -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_subnet_icmp_out -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_subnet_icmp_out -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_fwl_multi_icmp_out -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_fwl_multi_icmp_out -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_llcl_icmp_in -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_subnet_llcl_icmp_in -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_subnet_llcl_icmp_in -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_subnet_multi_icmp_in -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_subnet_multi_icmp_in -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_llcl_icmp_out -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_out -p icmpv6 --icmpv6-type echo-request -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_llcl_multi_icmp_in -p icmpv6 --icmpv6-type echo-request -m state --state NEW -j ACCEPT
fi
...
if [ "$ISA_DHCP6_CLIENT" = "1" ]; then
 ...
 # Initialization or rebinding: No lease or Lease time expired
	$IP6TABLES -A lcl_dhcp_client_query -s $EXT_LINKLOCAL_IP -d $MULTICAST -j ACCEPT
 # DHCP response
	$IP6TABLES -A rmt_dhcp_srv_response -s $LINK_LOCAL -d $EXT_LINKLOCAL_IP -j ACCEPT
fi
...
if [ "$ISA_DHCP6_SERVER" = "1" ]; then
 ...
 # Lease renewal 
	$IP6TABLES -A LAN_dhcp_client_query -i $LAN_IF -d $LOCAL_DHCP_SERVER_LINK_LOCAL -j ACCEPT
...
if [ "$ISA_DHCP6_CLIENT" = "1" ]; then
 if [ "$RUN_SILENTLY" != "1" ]; then
	...
	$IP6TABLES -A INPUT -i $INTERNET_IF -p udp --sport bootps --dport bootpc -j rmt_dhcp_srv_response
	$IP6TABLES -A OUTPUT -o $INTERNET_IF -p udp --sport bootpc --dport bootps -j lcl_dhcp_client_query
	$IP6TABLES -A INPUT -i $INTERNET_IF -p udp --sport $DHCP_IPV6_ADVERTISE_UDP_PORT --dport $DHCP_IPV6_SOLICIT_UDP_PRT -j rmt_dhcp_srv_response
	$IP6TABLES -A OUTPUT -o $INTERNET_IF -p udp --sport $DHCP_IPV6_SOLICIT_UDP_PRT --dport $DHCP_IPV6_ADVERTISE_UDP_PORT -j lcl_dhcp_client_query
fi
...
if [ "$ISA_DHCP6_SERVER" = "1" ]; then
 if [ "$RUN_SILENTLY" != "1" ]; then
	...
	$IP6TABLES -A INPUT -i $LAN_IF -p udp --sport bootpc --dport bootps -j LAN_dhcp_client_query
	$IP6TABLES -A OUTPUT -o $LAN_IF -p udp --sport bootps --dport bootpc -j LAN_dhcp_srv_response
	$IP6TABLES -A INPUT -i $LAN_IF -p udp --sport 21302 --dport 21302 -j LAN_dhcp_client_query
	$IP6TABLES -A OUTPUT -o $LAN_IF -p udp --sport 21302 --dport 21302 -j LAN_dhcp_srv_response
	$IP6TABLES -A INPUT -i $LAN_IF -p udp --sport $DHCP_IPV6_SOLICIT_UDP_PRT --dport $DHCP_IPV6_ADVERTISE_UDP_PORT -j LAN_dhcp_client_query
	$IP6TABLES -A OUTPUT -o $LAN_IF -p udp --sport $DHCP_IPV6_SOLICIT_UDP_PRT --dport $DHCP_IPV6_ADVERTISE_UDP_PORT -j LAN_dhcp_srv_response
...
# ICMP rules #
##############
$IP6TABLES -A log_in -p icmpv6 ! --icmpv6-type echo-request -m limit --limit 1/sec -j LOG --log-prefix "(IPv6)(D-PG)IN-drop: "
...
$IP6TABLES -A log_out -p icmpv6 --icmpv6-type destination-unreachable -j LOG --log-prefix "(IPv6)(D-PG)OUT-icmp-dest-unrch-drop: "
$IP6TABLES -A log_out -p icmpv6 --icmpv6-type destination-unreachable -j DROP
# End debug.
...
$IP6TABLES -A INPUT -i $INTERNET_IF -s $LINK_LOCAL -d $MULTICAST -j ext_if_llcl_multi_in
...
$IP6TABLES -A INPUT -i $INTERNET_IF -s $LINK_LOCAL -d $LINK_LOCAL -j ext_if_llcl_llcl_in
...
$IP6TABLES -A INPUT -i $LAN_IF -s $LINK_LOCAL -d $MULTICAST -j lan_if_llcl_multi_in
...
$IP6TABLES -A INPUT -i $LAN_IF -s $LINK_LOCAL -d $LINK_LOCAL -j lan_if_llcl_llcl_in
...
$IP6TABLES -A OUTPUT -o $INTERNET_IF -s $LINK_LOCAL -d $MULTICAST -j ext_if_llcl_multi_out
...
$IP6TABLES -A OUTPUT -o $INTERNET_IF -s $LINK_LOCAL -d $LINK_LOCAL -j ext_if_llcl_llcl_out
...
$IP6TABLES -A OUTPUT -o $LAN_IF -s $LINK_LOCAL -d $MULTICAST -j lan_if_llcl_multi_out
...
$IP6TABLES -A OUTPUT -o $LAN_IF -s $LINK_LOCAL -d $LINK_LOCAL -j lan_if_llcl_llcl_out
...
$IP6TABLES -A INPUT -i $LAN_IF -s $LAN_SUBNET -d $INT_LINKLOCAL_IP -j lan_if_subnet_llcl_in
...
$IP6TABLES -A INPUT -i $LAN_IF -s $LAN_SUBNET -d $MULTICAST -j lan_if_subnet_multi_in
...
$IP6TABLES -A OUTPUT -o $LAN_IF -s $INT_LINKLOCAL_IP -d $LAN_SUBNET -j lan_if_llcl_subnet_out
...
$IP6TABLES -A ext_if_out -p udp --sport domain --dport domain -j lcl_dns_srv_to_trust_dns_srv
$IP6TABLES -A ext_if_in -p udp --sport domain --dport domain -j lcl_dns_srv_fm_trust_dns_srv
$IP6TABLES -A ext_if_in -p udp --sport domain --dport $UNPRIVPORTS -j lcl_dns_srv_fm_trust_dns_srv
$IP6TABLES -A ext_if_out -p udp --sport domain-s --dport domain-s -j lcl_dns_srv_to_trust_dns_srv
$IP6TABLES -A ext_if_in -p udp --sport domain-s --dport domain-s -j lcl_dns_srv_fm_trust_dns_srv
$IP6TABLES -A ext_if_in -p udp --sport domain-s --dport $UNPRIVPORTS -j lcl_dns_srv_fm_trust_dns_srv
$IP6TABLES -A ext_if_out -p udp --sport $MULTICAST_DNS_UDP_PORT --dport $MULTICAST_DNS_UDP_PORT -j lcl_dns_srv_to_trust_dns_srv
$IP6TABLES -A ext_if_in -p udp --sport $MULTICAST_DNS_UDP_PORT --dport $MULTICAST_DNS_UDP_PORT -j lcl_dns_srv_fm_trust_dns_srv

# Set up the jump to the chains for TCP DNS requests
$IP6TABLES -A ext_if_out -p tcp --sport $UNPRIVPORTS --dport domain -j lcl_dns_srv_to_trust_dns_srv
$IP6TABLES -A ext_if_in -p tcp ! --syn --sport domain --dport $UNPRIVPORTS -j lcl_dns_srv_fm_trust_dns_srv
$IP6TABLES -A ext_if_out -p tcp --sport $UNPRIVPORTS --dport domain-s -j lcl_dns_srv_to_trust_dns_srv
$IP6TABLES -A ext_if_in -p tcp ! --syn --sport domain-s --dport $UNPRIVPORTS -j lcl_dns_srv_fm_trust_dns_srv
...
# from the firewall
$IP6TABLES -A lcl_dns_srv_to_trust_dns_srv -d $NAMESERVER_1 -j ACCEPT
$IP6TABLES -A lcl_dns_srv_to_trust_dns_srv -d $NAMESERVER_2 -j ACCEPT
$IP6TABLES -A lcl_dns_srv_to_trust_dns_srv -d $NAMESERVER_3 -j ACCEPT
$IP6TABLES -A lcl_dns_srv_to_trust_dns_srv -d $NAMESERVER_4 -j ACCEPT

$IP6TABLES -A lcl_dns_srv_fm_trust_dns_srv -s $NAMESERVER_1 -j ACCEPT
$IP6TABLES -A lcl_dns_srv_fm_trust_dns_srv -s $NAMESERVER_2 -j ACCEPT
$IP6TABLES -A lcl_dns_srv_fm_trust_dns_srv -s $NAMESERVER_3 -j ACCEPT
$IP6TABLES -A lcl_dns_srv_fm_trust_dns_srv -s $NAMESERVER_4 -j ACCEPT
...
$IP6TABLES -A lan_if_out -p udp --sport domain --dport $UNPRIVPORTS -j trusted_dns_srv_LAN_response
$IP6TABLES -A lan_if_out -p udp --sport domain-s --dport $UNPRIVPORTS -j trusted_dns_srv_LAN_response
$IP6TABLES -A lan_if_out -p udp --sport $MULTICAST_DNS_UDP_PORT --dport $MULTICAST_DNS_UDP_PORT -j trusted_dns_srv_LAN_response
$IP6TABLES -A lan_if_in -p udp --sport $UNPRIVPORTS --dport domain -j trusted_dns_srv_LAN_query
$IP6TABLES -A lan_if_in -p udp --sport $UNPRIVPORTS --dport domain-s -j trusted_dns_srv_LAN_query
$IP6TABLES -A lan_if_in -p udp --sport $MULTICAST_DNS_UDP_PORT --dport $MULTICAST_DNS_UDP_PORT -j trusted_dns_srv_LAN_query
$IP6TABLES -A ext_if_lan_out -p udp --sport domain --dport $UNPRIVPORTS -j trusted_dns_srv_LAN_response
$IP6TABLES -A ext_if_lan_out -p udp --sport domain-s --dport $UNPRIVPORTS -j trusted_dns_srv_LAN_response
$IP6TABLES -A ext_if_lan_out -p udp --sport $MULTICAST_DNS_UDP_PORT --dport $MULTICAST_DNS_UDP_PORT -j trusted_dns_srv_LAN_response
$IP6TABLES -A ext_if_lan_in -p udp --sport $UNPRIVPORTS --dport domain -j trusted_dns_srv_LAN_query
$IP6TABLES -A ext_if_lan_in -p udp --sport $UNPRIVPORTS --dport domain-s -j trusted_dns_srv_LAN_query
$IP6TABLES -A ext_int -p udp --sport domain --dport $UNPRIVPORTS -j trusted_dns_srv_LAN_response
$IP6TABLES -A ext_int -p udp --sport domain-s --dport $UNPRIVPORTS -j trusted_dns_srv_LAN_response
$IP6TABLES -A ext_int -p udp --sport $MULTICAST_DNS_UDP_PORT --dport $MULTICAST_DNS_UDP_PORT -j trusted_dns_srv_LAN_response
$IP6TABLES -A int_ext -p udp --sport $UNPRIVPORTS --dport domain -j trusted_dns_srv_LAN_query
$IP6TABLES -A int_ext -p udp --sport $UNPRIVPORTS --dport domain-s -j trusted_dns_srv_LAN_query
$IP6TABLES -A int_ext -p udp --sport $MULTICAST_DNS_UDP_PORT --dport $MULTICAST_DNS_UDP_PORT -j trusted_dns_srv_LAN_query

# Chains for TCP requests
$IP6TABLES -A lan_if_out -p tcp --sport domain --dport $UNPRIVPORTS -j trusted_dns_srv_LAN_response
$IP6TABLES -A lan_if_in -p tcp --sport $UNPRIVPORTS --dport domain -j trusted_dns_srv_LAN_query
$IP6TABLES -A ext_if_lan_out -p tcp --sport domain --dport $UNPRIVPORTS -j trusted_dns_srv_LAN_response
$IP6TABLES -A ext_if_lan_in -p tcp --sport $UNPRIVPORTS --dport domain -j trusted_dns_srv_LAN_query
$IP6TABLES -A lan_if_out -p tcp --sport domain-s --dport $UNPRIVPORTS -j trusted_dns_srv_LAN_response
$IP6TABLES -A lan_if_in -p tcp --sport $UNPRIVPORTS --dport domain-s -j trusted_dns_srv_LAN_query
$IP6TABLES -A ext_if_lan_out -p tcp --sport domain-s --dport $UNPRIVPORTS -j trusted_dns_srv_LAN_response
$IP6TABLES -A ext_if_lan_in -p tcp --sport $UNPRIVPORTS --dport domain-s -j trusted_dns_srv_LAN_query

####################
# Set up the rules #
####################
# Unlimited link local traffic on the LAN IF to our LAN's IP
$IP6TABLES -A INPUT -i $LAN_IF -s $LINK_LOCAL -d $LAN_IP -j ACCEPT
$IP6TABLES -A OUTPUT -o $LAN_IF -s $LAN_IP -d $LINK_LOCAL -j ACCEPT

# Add the rules for multicast requests and responses
$IP6TABLES -A trusted_dns_srv_LAN_query -p udp -s $LAN_IP -d $MULTICAST -j ACCEPT
$IP6TABLES -A trusted_dns_srv_LAN_response -p udp -s $MULTICAST -d $LAN_IP -j ACCEPT
$IP6TABLES -A trusted_dns_srv_LAN_query -p udp -s $INTERNET_IP -d $MULTICAST -j ACCEPT
$IP6TABLES -A trusted_dns_srv_LAN_response -p udp -s $MULTICAST -d $INTERNET_IP -j ACCEPT
$IP6TABLES -A OUTPUT -o $LAN_IF -s $LAN_IP -d $MULTICAST -j lan_if_fwl_multi_out

# Add the rules for DNS requests going to trusted DNS servers
$IP6TABLES -A trusted_dns_srv_LAN_query -d $LOCAL_NAMESERVER -j ACCEPT
$IP6TABLES -A trusted_dns_srv_LAN_query -d $LOCAL_NAMESERVER_INT -j ACCEPT
$IP6TABLES -A trusted_dns_srv_LAN_query -d $NAMESERVER_1 -j ACCEPT
$IP6TABLES -A trusted_dns_srv_LAN_query -d $NAMESERVER_2 -j ACCEPT
$IP6TABLES -A trusted_dns_srv_LAN_query -d $NAMESERVER_3 -j ACCEPT
$IP6TABLES -A trusted_dns_srv_LAN_query -d $NAMESERVER_4 -j ACCEPT

# Add the rules for DNS reponses coming from trusted DNS servers
$IP6TABLES -A trusted_dns_srv_LAN_response -s $LOCAL_NAMESERVER -j ACCEPT
$IP6TABLES -A trusted_dns_srv_LAN_response -s $LOCAL_NAMESERVER_INT -j ACCEPT
$IP6TABLES -A trusted_dns_srv_LAN_response -s $NAMESERVER_1 -j ACCEPT
$IP6TABLES -A trusted_dns_srv_LAN_response -s $NAMESERVER_2 -j ACCEPT
$IP6TABLES -A trusted_dns_srv_LAN_response -s $NAMESERVER_3 -j ACCEPT
$IP6TABLES -A trusted_dns_srv_LAN_response -s $NAMESERVER_4 -j ACCEPT
...
$IP6TABLES -A ext_if_out -p udp --sport $UNPRIVPORTS --dport domain -j rmt_dns_srv_query
$IP6TABLES -A ext_if_out -p udp --sport $UNPRIVPORTS --dport domain-s -j rmt_dns_srv_query
$IP6TABLES -A ext_if_in -p udp --sport domain --dport $UNPRIVPORTS -j rmt_dns_srv_response
$IP6TABLES -A ext_if_in -p udp --sport domain-s --dport $UNPRIVPORTS -j rmt_dns_srv_response

# Set up the jump to the chains for TCP requests
$IP6TABLES -A ext_if_out -p tcp --sport $UNPRIVPORTS --dport domain -j rmt_dns_srv_query
$IP6TABLES -A ext_if_out -p tcp --sport $UNPRIVPORTS --dport domain-s -j rmt_dns_srv_query
$IP6TABLES -A ext_if_in -p tcp ! --syn --sport domain --dport $UNPRIVPORTS -j rmt_dns_srv_response
$IP6TABLES -A ext_if_in -p tcp ! --syn --sport domain-s --dport $UNPRIVPORTS -j rmt_dns_srv_response
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A rmt_dns_srv_response -p udp -m state --state ESTABLISHED,RELATED -j ACCEPT
fi
...
#####################
# Chains for UDP requests
$IP6TABLES -A ext_if_in -p udp --sport $UNPRIVPORTS --dport domain -j lcl_dns_srv_rmt_query
$IP6TABLES -A ext_if_out -p udp --sport domain --dport $UNPRIVPORTS -j lcl_dns_srv_rmt_response
$IP6TABLES -A ext_if_in -p udp --sport domain --dport domain -j lcl_dns_srv_rmt_query
$IP6TABLES -A ext_if_out -p udp --sport domain --dport domain -j lcl_dns_srv_rmt_response
$IP6TABLES -A ext_if_in -p udp --sport $UNPRIVPORTS --dport domain-s -j lcl_dns_srv_rmt_query
$IP6TABLES -A ext_if_out -p udp --sport domain-s --dport $UNPRIVPORTS -j lcl_dns_srv_rmt_response
$IP6TABLES -A ext_if_in -p udp --sport domain-s --dport domain-s -j lcl_dns_srv_rmt_query
$IP6TABLES -A ext_if_out -p udp --sport domain-s --dport domain-s -j lcl_dns_srv_rmt_response

# Chains for TCP requests
$IP6TABLES -A ext_if_in -p tcp --syn --sport $UNPRIVPORTS --dport domain -j lcl_dns_srv_rmt_query
$IP6TABLES -A ext_if_out -p tcp --syn --sport domain --dport $UNPRIVPORTS -j lcl_dns_srv_rmt_response
$IP6TABLES -A ext_if_in -p tcp --syn --sport $UNPRIVPORTS --dport domain-s -j lcl_dns_srv_rmt_query
$IP6TABLES -A ext_if_out -p tcp --syn --sport domain-s --dport $UNPRIVPORTS -j lcl_dns_srv_rmt_response
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
 # Add the rules for DNS requests from the Internet
	$IP6TABLES -A lcl_dns_srv_rmt_query -d $LOCAL_NAMESERVER -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lcl_tcp_client_request -p tcp --dport ssh --syn -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A lcl_tcp_client_request -p tcp --dport ssh -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p tcp ! --syn --sport ssh -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lcl_tcp_client_request -p tcp -m multiport --destination-port http,whois,$WEB_ALT_PORT,$WEB_PROXY_PORT,$WEB_PROXY_PORT2,$WEB_ALT_HTTPS_PORT,$VT_ALT_HTTPS_PORT,https,auth,ftp,ftp-data,ftps,ftps-data,4885 --syn -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A lcl_tcp_client_request -p tcp -m multiport --destination-port http,$WEB_ALT_PORT,$WEB_PROXY_PORT,$WEB_PROXY_PORT2,$WEB_ALT_HTTPS_PORT,https,auth,ftp,ftp-data,ftps,ftps-data -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p tcp -m multiport --source-port http,$WEB_ALT_PORT,$WEB_PROXY_PORT,$WEB_PROXY_PORT2,$WEB_ALT_HTTPS_PORT,https,auth,ftp,ftp-data,ftps,ftps-data -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p udp --source-port https -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p tcp --sport ftp-data -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p tcp --sport ftps-data -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lcl_tcp_client_request -p tcp -m multiport --destination-port pop3,imap2 --syn -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A lcl_tcp_client_request -p tcp -m multiport --destination-port pop3,imap2 -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p tcp -m multiport --source-port pop3,imap2 ! --syn -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lcl_tcp_client_request -p tcp --dport pop3s --syn -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A lcl_tcp_client_request -p tcp --dport pop3s -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p tcp ! --syn --sport pop3s -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lcl_tcp_client_request -p tcp --dport imaps --syn -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A lcl_tcp_client_request -p tcp --dport imaps -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p tcp ! --syn --sport imaps -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lcl_tcp_client_request -p tcp -d $REMOTE_SMTP_SERVER --dport smtp --syn -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A lcl_tcp_client_request -p tcp -d $REMOTE_SMTP_SERVER --dport smtp -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p tcp ! --syn -s $REMOTE_SMTP_SERVER --sport smtp -j ACCEPT
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lcl_tcp_client_request -p tcp -d $REMOTE_SMTP_SERVER --dport smtps --syn -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A lcl_tcp_client_request -p tcp -d $REMOTE_SMTP_SERVER --dport smtps -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p tcp ! --syn -s $REMOTE_SMTP_SERVER --sport smtps -j ACCEPT
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lcl_tcp_client_request -p tcp -d $REMOTE_SMTP_SERVER --dport $ALT_SMTPS_PORT --syn -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A lcl_tcp_client_request -p tcp -d $REMOTE_SMTP_SERVER --dport $ALT_SMTPS_PORT -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p tcp ! --syn -s $REMOTE_SMTP_SERVER --sport $ALT_SMTPS_PORT -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lcl_tcp_client_request -p tcp -d $NEWS_SERVER --dport snmp --syn -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A lcl_tcp_client_request -p tcp -d $NEWS_SERVER --dport snmp -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p tcp ! --syn -s $NEWS_SERVER --sport snmp -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lcl_tcp_client_request -p tcp -d $NEWS_SERVER --dport nntp --syn -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A lcl_tcp_client_request -p tcp -d $NEWS_SERVER --dport nntp -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p tcp ! --syn -s $NEWS_SERVER --sport nntp -j ACCEPT
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lcl_tcp_client_request -p tcp -d $NEWS_SERVER --dport nntps --syn -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A lcl_tcp_client_request -p tcp -d $NEWS_SERVER --dport nntps -j ACCEPT
$IP6TABLES -A rmt_tcp_srv_response -p tcp ! --syn -s $NEWS_SERVER --sport nntps -j ACCEPT
...
$IP6TABLES -A lan_if_in -p udp -s $LAN_SUBNET --sport $UNPRIVPORTS -j LAN_udp_client_request
$IP6TABLES -A lan_if_out -p udp --dport $UNPRIVPORTS -j udp_srv_LAN_response
$IP6TABLES -A lan_if_in -p udp -s $LAN_SUBNET --sport $MULTICAST_DNS_UDP_PORT -j LAN_udp_client_request
$IP6TABLES -A lan_if_out -p udp --dport $MULTICAST_DNS_UDP_PORT -j udp_srv_LAN_response

# Multicast DNS
if [ "$RUN_SILENTLY" != "1" ]; then
	echo "& Multicast DNS "
fi
$IP6TABLES -A LAN_udp_client_request -p udp --dport $MULTICAST_DNS_UDP_PORT -j ACCEPT
$IP6TABLES -A udp_srv_LAN_response -p udp --sport $MULTICAST_DNS_UDP_PORT -j ACCEPT

# Some authentication servers
$IP6TABLES -A LAN_udp_client_request -p udp --destination-port https -j ACCEPT
$IP6TABLES -A udp_srv_LAN_response -p udp --source-port https -j ACCEPT
...
$IP6TABLES -A lan_if_in -p tcp -s $LAN_SUBNET --sport $UNPRIVPORTS -j LAN_tcp_client_request
$IP6TABLES -A lan_if_out -p tcp ! --syn -d $LAN_SUBNET --dport $UNPRIVPORTS -j tcp_srv_LAN_response
# special rule to allow ssh to new server machine at its temp IP addr
$IP6TABLES -A lan_if_out -p tcp -s $LAN_IP --sport $UNPRIVPORTS -d $LAN_SUBNET --dport ssh -j ACCEPT
...
$IP6TABLES -A ext_if_lan_in -p tcp -s $LAN_SUBNET --sport $UNPRIVPORTS -j LAN_tcp_client_request
$IP6TABLES -A ext_if_lan_out -p tcp ! --syn -d $LAN_SUBNET --dport $UNPRIVPORTS -j tcp_srv_LAN_response
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A LAN_tcp_client_request -p tcp --dport ssh --syn -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A LAN_tcp_client_request -p tcp --dport ssh -j ACCEPT
$IP6TABLES -A tcp_srv_LAN_response -p tcp ! --syn --sport ssh -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A LAN_tcp_client_request -p tcp -m multiport --destination-port http,https,auth --syn -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A LAN_tcp_client_request -p tcp -m multiport --destination-port http,https,auth -j ACCEPT
$IP6TABLES -A tcp_srv_LAN_response -p tcp -m multiport --source-port http,https,auth ! --syn -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A rmt_tcp_client_request -p tcp --dport ssh -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A rmt_tcp_client_request -p tcp --dport ssh -j ACCEPT
$IP6TABLES -A lcl_tcp_srv_response -p tcp ! --syn --sport ssh -j ACCEPT
...
# necessary.
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A local_udp_client_request -p udp --sport ntp --dport ntp -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A local_udp_client_request -p udp --sport $UNPRIVPORTS --dport ntp -j ACCEPT
$IP6TABLES -A remote_udp_srv_response -p udp --sport ntp --dport $UNPRIVPORTS -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A local_udp_client_request -p udp --sport $UNPRIVPORTS --dport snmp:snmptrap -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A local_udp_client_request -p udp --sport $UNPRIVPORTS --dport snmp:snmptrap -j ACCEPT
$IP6TABLES -A remote_udp_srv_response -p udp --sport snmp:snmptrap --dport $UNPRIVPORTS -j ACCEPT

# HTTPS - used by virtually every website today
if [ "$RUN_SILENTLY" != "1" ]; then
	echo -n "https, "
fi
$IP6TABLES -A local_udp_client_request -p udp --sport $UNPRIVPORTS --destination-port https -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A local_udp_client_request -p udp --sport $TRACEROUTE_SRC_PORTS --dport $TRACEROUTE_DEST_PORTS -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A local_udp_client_request -p udp --sport $TRACEROUTE_SRC_PORTS --dport $TRACEROUTE_DEST_PORTS -j ACCEPT
$IP6TABLES -A remote_udp_srv_response -p udp --sport $TRACEROUTE_DEST_PORTS --dport $TRACEROUTE_SRC_PORTS -j ACCEPT

# Multicast DNS
if [ "$RUN_SILENTLY" != "1" ]; then
	echo "(IPv6) & Mutlicast DNS "
fi
$IP6TABLES -A local_udp_client_request -p udp --dport $MULTICAST_DNS_UDP_PORT -j ACCEPT
$IP6TABLES -A remote_udp_srv_response -p udp --sport $MULTICAST_DNS_UDP_PORT -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A LAN_udp_firewall_request -p udp -s $LAN_IP --sport $UNPRIVPORTS -d $PRINTER --dport $PRINTER_CTRL_PORT1 -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A LAN_udp_firewall_request -p udp -s $LAN_IP --sport $UNPRIVPORTS -d $PRINTER --dport $PRINTER_CTRL_PORT1 -j ACCEPT
$IP6TABLES -A LAN_udp_firewall_response -p udp -s $PRINTER --sport $PRINTER_CTRL_PORT1 -d $LAN_IP --dport $UNPRIVPORTS -j ACCEPT
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A LAN_udp_firewall_request -p udp --sport $UNPRIVPORTS -d $PRINTER --dport $PRINTER_CTRL_PORT2 -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A LAN_udp_firewall_request -p udp --sport $UNPRIVPORTS -d $PRINTER --dport $PRINTER_CTRL_PORT2 -j ACCEPT
$IP6TABLES -A LAN_udp_firewall_response -p udp -s $PRINTER --sport $PRINTER_CTRL_PORT2 --dport $UNPRIVPORTS -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A LAN_tcp_firewall_request -p tcp -s $LAN_IP --sport $UNPRIVPORTS -d $PRINTER --dport $PRINTER_TCP_PRINTING -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A LAN_tcp_firewall_request -p tcp -s $LAN_IP --sport $UNPRIVPORTS -d $PRINTER --dport $PRINTER_TCP_PRINTING -j ACCEPT
$IP6TABLES -A LAN_tcp_firewall_response -p tcp -s $PRINTER --sport $PRINTER_TCP_PRINTING -d $LAN_IP --dport $UNPRIVPORTS -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A remote_udp_client_request -p udp --sport $TRACEROUTE_SRC_PORTS --dport $TRACEROUTE_DEST_PORTS -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A remote_udp_client_request -p udp --sport $TRACEROUTE_SRC_PORTS --dport $TRACEROUTE_DEST_PORTS -j ACCEPT
$IP6TABLES -A local_udp_srv_response -p udp --sport $TRACEROUTE_DEST_PORTS --dport $TRACEROUTE_SRC_PORTS -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_icmp_out -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_icmp_in -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_icmp_in -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_icmp_out -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_if_icmp_out -p icmpv6 --icmpv6-type echo-request -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_icmp_in -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_icmp_out -p icmpv6 --icmpv6-type neighbor-advertisement -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_icmp_in -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_icmp_out -p icmpv6 --icmpv6-type neighbor-solicitation -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_icmp_out -p icmpv6 --icmpv6-type echo-request -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A lan_if_icmp_in -p icmpv6 -s $LAN_SUBNET --icmpv6-type echo-request -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A lan_if_icmp_in -p icmpv6 --icmpv6-type echo-request -s $LAN_SUBNET -j ACCEPT
$IP6TABLES -A lan_if_icmp_out -p icmpv6 --icmpv6-type echo-reply -d $LAN_SUBNET -j ACCEPT
...
# fi
# if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
# $IP6TABLES -A int_ext_icmp -s $LINK_LOCAL -d $MULTICAST -p icmpv6 --icmpv6-type router-solicitation # -m state --state NEW # -j ACCEPT
# fi
...
# fi
# if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
# $IP6TABLES -A ext_int_icmp -s $LINK_LOCAL -d $MULTICAST -p icmpv6 --icmpv6-type router-advertisement # -m state --state NEW # -j ACCEPT
# fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A int_ext_icmp -p icmpv6 --icmpv6-type echo-request -m state --state NEW -j ACCEPT
fi
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A ext_int_icmp -p icmpv6 --icmpv6-type echo-request -m state --state NEW -j ACCEPT
fi
...
$IP6TABLES -A vpn_machine_out -p udp --sport isakmp --dport isakmp -j ACCEPT
$IP6TABLES -A vpn_machine_out -p tcp --sport $UNPRIVPORTS --dport $VPN_SERVICE_TCP_PORT -j ACCEPT
$IP6TABLES -A vpn_machine_out -p tcp --sport $UNPRIVPORTS --dport $VPN_SERVICE_TCP_PORT_RNG -j ACCEPT
$IP6TABLES -A vpn_machine_out -p tcp --sport $UNPRIVPORTS --dport $VPN_SERVICE3_TCP_PORT_RNG -j ACCEPT
$IP6TABLES -A vpn_machine_out -p 47 -j ACCEPT
$IP6TABLES -A vpn_machine_out -p udp --sport $UNPRIVPORTS --dport $VPN_SERVICE2_UDP_PORT -j ACCEPT
$IP6TABLES -A vpn_machine_out -p udp --sport $UNPRIVPORTS --dport $VPN_SERVICE3_UDP_PORT2 -j ACCEPT
$IP6TABLES -A vpn_machine_out -p udp --sport $VPN_SERVICE3_UDP_PORT --dport $VPN_SERVICE3_UDP_PORT -j ACCEPT
$IP6TABLES -A vpn_machine_out -p udp --sport $UNPRIVPORTS --dport $VPN_SERVICE3_UDP_PORT_RNG -j ACCEPT
$IP6TABLES -A vpn_machine_out -p udp --sport $UNPRIVPORTS -d $VPN2_IP --dport isakmp -j ACCEPT
$IP6TABLES -A vpn_machine_out -p udp --sport $UNPRIVPORTS -d $VPN3_IP --dport isakmp -j ACCEPT
$IP6TABLES -A vpn_machine_out -p udp --sport $UNPRIVPORTS --dport domain -j ACCEPT
$IP6TABLES -A vpn_machine_out -p udp --sport $UNPRIVPORTS -d $VPN2_IP --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A vpn_machine_out -p udp --sport $UNPRIVPORTS -d $VPN3_IP --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A vpn_machine_out -p udp --sport $UNPRIVPORTS --dport isakmp -j ACCEPT
$IP6TABLES -A vpn_machine_out -p udp --sport $VPN_SERVICE_UDP_PORT --dport $VPN_SERVICE_UDP_PORT -j ACCEPT
$IP6TABLES -A vpn_machine_out -p tcp --sport $UNPRIVPORTS -d $VPN_IP --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A vpn_machine_out -p tcp --sport $UNPRIVPORTS -d $VPN2_IP --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A vpn_machine_out -p tcp --sport $UNPRIVPORTS -d $VPN3_IP --dport $VPN_SERVICE3_TCP_PORT -j ACCEPT
$IP6TABLES -A vpn_machine_out -p tcp --sport $UNPRIVPORTS --dport $VPN_SERVICE3_TCP_PORT2 -j ACCEPT
$IP6TABLES -A vpn_machine_out -p tcp --sport $UNPRIVPORTS --dport $VPN_SERVICE3_TCP_PORT3_RNG -j ACCEPT
$IP6TABLES -A vpn_machine_out -p tcp --sport $UNPRIVPORTS --dport $VPN_SERVICE3_TCP_PORT4 -j ACCEPT

$IP6TABLES -A vpn_machine_in -p esp -j ACCEPT
$IP6TABLES -A vpn_machine_in -p udp --sport isakmp --dport isakmp -j ACCEPT
$IP6TABLES -A vpn_machine_in -p udp --sport $VPN_SERVICE_UDP_PORT --dport $VPN_SERVICE_UDP_PORT -j ACCEPT
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
	$IP6TABLES -A vpn_machine_out -p tcp --dport $VPN_CLIENT_MAIL_PORTS -m state --state NEW -j ACCEPT
fi
$IP6TABLES -A vpn_machine_out -p tcp --dport $VPN_CLIENT_MAIL_PORTS -j ACCEPT
$IP6TABLES -A vpn_machine_in -p tcp ! --syn --sport $VPN_CLIENT_MAIL_PORTS -j ACCEPT
...
GW2_TCP_PORT1="6112"
GW2_TCP_PORT2="6600"
$IP6TABLES -A guest_gamer_out -p tcp --sport $GW2_TCP_PORT1 --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A guest_gamer_out -p tcp --sport $GW2_TCP_PORT2 --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A guest_gamer_out -p tcp --sport $UNPRIVPORTS --dport $GW2_TCP_PORT1 -j ACCEPT
$IP6TABLES -A guest_gamer_out -p tcp --sport $UNPRIVPORTS --dport $GW2_TCP_PORT2 -j ACCEPT
$IP6TABLES -A guest_gamer_in -p tcp --sport $UNPRIVPORTS --dport $GW2_TCP_PORT1 -j ACCEPT
$IP6TABLES -A guest_gamer_in -p tcp --sport $UNPRIVPORTS --dport $GW2_TCP_PORT2 -j ACCEPT

IRC_TCP_PORT_RNG1="6660:6669"
IRC_TCP_PORT_RNG2="5000:5009"
$IP6TABLES -A guest_gamer_out -p tcp --sport $UNPRIVPORTS --dport $IRC_TCP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A guest_gamer_out -p tcp --sport $UNPRIVPORTS --dport $IRC_TCP_PORT_RNG2 -j ACCEPT
...
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $LOL_UDP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A guest_gamer_out -p tcp --sport $UNPRIVPORTS --dport $LOL_TCP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $LOL_UDP_PORT1 -j ACCEPT
$IP6TABLES -A guest_gamer_out -p tcp --sport $UNPRIVPORTS -m multiport --dports $LOL_TCP_PORT_LIST1 -j ACCEPT
...
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS -m multiport --dports $PAYDAY2_UDP_PORT_LIST1 -j ACCEPT
...
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $ROBLOX_UDP_PORT_RNG -j ACCEPT
...
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $KF_GAME_UDP_PORT -j ACCEPT
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $KF_QUERY_UDP_PORT -j ACCEPT
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $KF2_GAME_TCP_UDP_PORT -j ACCEPT
$IP6TABLES -A guest_gamer_out -p tcp --sport $UNPRIVPORTS --dport $KF2_GAME_TCP_UDP_PORT -j ACCEPT
$IP6TABLES -A guest_gamer_out -p tcp --sport $UNPRIVPORTS --dport $KF_WEBADMIN_TCP_PORT -j ACCEPT
$IP6TABLES -A guest_gamer_out -p tcp --sport $UNPRIVPORTS --dport $KF2_WEBADMIN_TCP_PORT -j ACCEPT
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $GAMESPY_QUERY_UDP_PORT_RNG -j ACCEPT
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $STEAM_UDP_PORT -j ACCEPT
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $KF_MASTER_SVR_UDP_PORT -j ACCEPT
$IP6TABLES -A guest_gamer_out -p tcp --sport $UNPRIVPORTS --dport $KF_MASTER_SVR_TCP_PORT -j ACCEPT
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $KF2_MASTER_SVR_TCP_UDP_PORT -j ACCEPT
$IP6TABLES -A guest_gamer_out -p tcp --sport $UNPRIVPORTS --dport $KF2_MASTER_SVR_TCP_UDP_PORT -j ACCEPT
...
$IP6TABLES -A guest_gamer_out -p udp --sport $STEAM_SVR_SEARCH_UDP_PORT2 --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $STEAM_FRIENDS_UDP_PORT -j ACCEPT
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $STEAM_UDP_PORT_RNG -j ACCEPT
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $STEAM_UDP_PORT_RNG2 -j ACCEPT
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $STEAM_UDP_PORT_RNG3 -j ACCEPT
$IP6TABLES -A guest_gamer_out -p tcp --sport $UNPRIVPORTS --dport $STEAM_TCP_PORT_RNG -j ACCEPT
$IP6TABLES -A guest_gamer_out -p tcp --sport $UNPRIVPORTS --dport $STEAM_TCP_CROSS_CONNECT_PORT -j ACCEPT
$IP6TABLES -A guest_gamer_out -p udp --sport $UNPRIVPORTS --dport $STEAM_DED_SVR_UDP_PORT1 -j ACCEPT
$IP6TABLES -A guest_gamer_in -p udp --sport $STEAM_UDP_SRC_PORT --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A guest_gamer_in -p udp --sport $UNPRIVPORTS --dport $STEAM_DED_SVR_UDP_PORT1 -j ACCEPT
$IP6TABLES -A guest_gamer_in -p udp --sport $UNPRIVPORTS --dport $STEAM_DED_SVR_UDP_PORT2 -j ACCEPT
...
$IP6TABLES -A network_printer_out -p tcp --sport $UNPRIVPORTS --dport $GOOGLE_TALK_TCP_PORT -j ACCEPT
...
$IP6TABLES -A laptop_out -p udp --sport $UNPRIVPORTS --dport $HANGOUTS_TCP_UDP_PORT_RNG -j ACCEPT
$IP6TABLES -A laptop_out -p tcp --sport $UNPRIVPORTS --dport $HANGOUTS_TCP_UDP_PORT_RNG -j ACCEPT
$IP6TABLES -A laptop_out -p tcp --sport $UNPRIVPORTS --dport $GOOGLE_PLAYSTORE_TCP_PORT -j ACCEPT
...
IRC_TCP_PORT_RNG1="6660:6669"
IRC_TCP_PORT_RNG2="5000:5009"
$IP6TABLES -A laptop_out -p tcp --sport $UNPRIVPORTS --dport $IRC_TCP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A laptop_out -p tcp --sport $UNPRIVPORTS --dport $IRC_TCP_PORT_RNG2 -j ACCEPT
...
STEAM_DED_SVR_UDP_PORT1="27015"
STEAM_DED_SVR_UDP_PORT2="27020"
$IP6TABLES -A laptop_out -p udp --sport $STEAM_SVR_SEARCH_UDP_PORT2 --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A laptop_out -p udp --sport $UNPRIVPORTS --dport $STEAM_FRIENDS_UDP_PORT -j ACCEPT
$IP6TABLES -A laptop_out -p udp --sport $UNPRIVPORTS --dport $STEAM_UDP_PORT_RNG -j ACCEPT
$IP6TABLES -A laptop_out -p udp --sport $UNPRIVPORTS --dport $STEAM_UDP_PORT_RNG2 -j ACCEPT
$IP6TABLES -A laptop_out -p udp --sport $UNPRIVPORTS --dport $STEAM_UDP_PORT_RNG3 -j ACCEPT
$IP6TABLES -A laptop_out -p tcp --sport $UNPRIVPORTS --dport $STEAM_TCP_PORT_RNG -j ACCEPT
$IP6TABLES -A laptop_out -p tcp --sport $UNPRIVPORTS --dport $STEAM_TCP_PORT_FRIENDS_LIST -j ACCEPT
$IP6TABLES -A laptop_out -p tcp --sport $UNPRIVPORTS --dport $STEAM_TCP_CROSS_CONNECT_PORT -j ACCEPT
$IP6TABLES -A laptop_out -p udp --sport $UNPRIVPORTS --dport $STEAM_DED_SVR_UDP_PORT1 -j ACCEPT
$IP6TABLES -A laptop_in -p udp --sport $STEAM_UDP_SRC_PORT --dport $UNPRIVPORTS -j ACCEPT
...
$IP6TABLES -A samsungphone_out -p udp --sport $UNPRIVPORTS --dport $ROBLOX_UDP_PORT -j ACCEPT
$IP6TABLES -A samsungphone_out -p udp --sport $UNPRIVPORTS --dport $ROBLOX_UDP_PORT_RANGE -j ACCEPT

# DNS over TLS from my cell phones
if [ "$RUN_SILENTLY" != "1" ]; then
	echo -n "DNS over TLS, "
fi
$IP6TABLES -A samsungphone_out -p tcp --sport $UNPRIVPORTS --dport domain-s -j ACCEPT

# Google services accessed from my cell phones
if [ "$RUN_SILENTLY" != "1" ]; then
	echo "& Google Playstore, "
fi
$IP6TABLES -A samsungphone_out -p tcp --sport $UNPRIVPORTS --dport $GOOGLE_PLAYSTORE_TCP_PORT -j ACCEPT
$IP6TABLES -A samsungphone_out -p tcp --sport $UNPRIVPORTS --dport $GOOGLE_PLAYSTORE_TCP_PORT -j ACCEPT
...
#
if [ "$RUN_SILENTLY" != "1" ]; then
	echo "(IPv6)Google Play, Roblox, Zooba, Among Us, Head Ball, "
fi
...
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $JACK_GAME_TCP_PORT1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $TA_GOOGLE_PLAY_TCP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $TA_GOOGLE_PLAY_TCP_PORT1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $TA_GOOGLE_PLAY_TCP_UDP_PORT1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $TA_GOOGLE_PLAY_TCP_UDP_PORT_RNG -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $TA_GOOGLE_PLAY_TCP_UDP_PORT_RNG -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $TA_GOOGLE_PLAY_TCP_UDP_PORT_RNG2 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $TA_GOOGLE_PLAY_TCP_UDP_PORT_RNG2 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $TA_GOOGLE_PLAY_PURCHASING_TCP_PORT -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $TA_GOOGLE_PLAY_TEAM_CHAT_TCP_PORT -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $TA_AGAR_TCP_PORT1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $ROBLOX_UDP_PORT -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $ROBLOX_UDP_PORT_RANGE -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $KING_OF_THIEVES_TCP_PORT1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $DRIVE_AHEAD_TCP_PORT1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $DRAGON_GAME_TCP_PORT1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $DRAGON_GAME_TCP_PORT2 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $DRAGON_GAME_TCP_PORT3 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $DRAGON_GAME_TCP_PORT4 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $DRAGON_GAME_UDP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $AMONG_US_UDP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $AMONG_US_UDP_PORT_RNG2 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $AMONG_US_UDP_PORT_RNG3 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $AMONG_US_UDP_PORT_RNG4 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $DEVIL_AMONG_US_TCP_PORT1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $HEAD_BALL_UDP_PORT1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $FB_MESSENGER_KIDS_UDP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $FB_MESSENGER_KIDS_UDP_PORT_RNG2 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $FB_MESSENGER_KIDS_TCP_PORT1 -j ACCEPT
...
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $MINECRAFT_TCP_PORT_1 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $MINECRAFT_TCP_PORT_2 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $MINECRAFT_TCP_PORT_3 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $MINECRAFT_TCP_PORT_4 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $MINECRAFT_TCP_PORT_5 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p tcp --sport $UNPRIVPORTS --dport $MINECRAFT_TCP_PORT_6 -j ACCEPT
$IP6TABLES -A galaxytablet_out -p udp --sport $UNPRIVPORTS --dport $MINECRAFT_UDP_PORT_RNG1 -j ACCEPT
...
$IP6TABLES -A wap_out -p tcp --sport $UNPRIVPORTS --dport daytime -j ACCEPT
$IP6TABLES -A wap_in -p tcp --sport daytime --dport $UNPRIVPORTS -j ACCEPT
...
$IP6TABLES -A wap_out -p tcp --sport $UNPRIVPORTS --dport $ORBI_UPDATE_SVC_PORT -j ACCEPT
...
$IP6TABLES -A wap_out -p tcp --sport $UNPRIVPORTS --dport domain -j ACCEPT
$IP6TABLES -A wap_out -p tcp --sport $UNPRIVPORTS --dport domain-s -j ACCEPT
...
$IP6TABLES -A roku_out -p udp --sport $UNPRIVPORTS --dport $ROKU_UDP_PORT1 -j ACCEPT
$IP6TABLES -A roku_out -p udp --sport $UNPRIVPORTS --dport $ROKU_UDP_PORT2 -j ACCEPT
$IP6TABLES -A roku_out -p tcp --sport $UNPRIVPORTS --dport $ROKU_TCP_PORT1 -j ACCEPT
$IP6TABLES -A roku_out -p tcp --sport $UNPRIVPORTS -d $ROKU_GOOGLE_DNS_1 --dport domain -j ACCEPT
$IP6TABLES -A roku_out -p tcp --sport $UNPRIVPORTS -d $ROKU_GOOGLE_DNS_2 --dport domain -j ACCEPT
$IP6TABLES -A roku_out -p tcp --sport $UNPRIVPORTS -d $ROKU_GOOGLE_DNS_1 --dport domain-s -j ACCEPT
$IP6TABLES -A roku_out -p tcp --sport $UNPRIVPORTS -d $ROKU_GOOGLE_DNS_2 --dport domain-s -j ACCEPT
...
$IP6TABLES -A switch_out -p tcp --sport $UNPRIVPORTS --dport $MINECRAFT_TCP_PORT_1 -j ACCEPT
$IP6TABLES -A switch_out -p tcp --sport $UNPRIVPORTS --dport $MINECRAFT_TCP_PORT_2 -j ACCEPT
$IP6TABLES -A switch_out -p tcp --sport $UNPRIVPORTS --dport $MINECRAFT_TCP_PORT_3 -j ACCEPT
$IP6TABLES -A switch_out -p tcp --sport $UNPRIVPORTS --dport $MINECRAFT_TCP_PORT_4 -j ACCEPT
$IP6TABLES -A switch_out -p tcp --sport $UNPRIVPORTS --dport $MINECRAFT_TCP_PORT_5 -j ACCEPT
$IP6TABLES -A switch_out -p tcp --sport $UNPRIVPORTS --dport $MINECRAFT_TCP_PORT_6 -j ACCEPT
$IP6TABLES -A switch_out -p udp --sport $UNPRIVPORTS --dport $UNPRIVPORTS -j ACCEPT
...
$IP6TABLES -A ecobee3_out -p tcp --sport $UNPRIVPORTS --dport $ECOBEE3_TCP_PORT1 -j ACCEPT
$IP6TABLES -A ecobee3_out -p tcp --sport $UNPRIVPORTS --dport $ECOBEE3_TCP_PORT_RNG1 -j ACCEPT
...
STEAM_DED_SVR_UDP_PORT1="27015"
STEAM_DED_SVR_UDP_PORT2="27020"
$IP6TABLES -A steamdeck_out -p udp --sport $STEAM_SVR_SEARCH_UDP_PORT2 --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A steamdeck_out -p udp --sport $UNPRIVPORTS --dport $STEAM_FRIENDS_UDP_PORT -j ACCEPT
$IP6TABLES -A steamdeck_out -p udp --sport $UNPRIVPORTS --dport $STEAM_UDP_PORT_RNG -j ACCEPT
$IP6TABLES -A steamdeck_out -p udp --sport $UNPRIVPORTS --dport $STEAM_UDP_PORT_RNG2 -j ACCEPT
$IP6TABLES -A steamdeck_out -p udp --sport $UNPRIVPORTS --dport $STEAM_UDP_PORT_RNG3 -j ACCEPT
$IP6TABLES -A steamdeck_out -p tcp --sport $UNPRIVPORTS --dport $STEAM_TCP_PORT_RNG -j ACCEPT
$IP6TABLES -A steamdeck_out -p tcp --sport $UNPRIVPORTS --dport $STEAM_TCP_PORT_FRIENDS_LIST -j ACCEPT
$IP6TABLES -A steamdeck_in -p udp --sport $UNPRIVPORTS --dport $STEAM_DED_SVR_UDP_RNG -j ACCEPT
$IP6TABLES -A steamdeck_in -p udp --sport $STEAM_UDP_SRC_PORT --dport $UNPRIVPORTS -j ACCEPT
...
# Constants 
EL_TCP_PORT1="5222"
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $EL_TCP_PORT1 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $VNC_TCP_PORT_RANGE -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ME_TCP_PORT1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ME_TCP_PORT2 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ME_TCP_PORT_RANGE_1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ME_TCP_PORT_RANGE_2 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ME2_TCP_PORT1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ME2_TCP_PORT2 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ME2_TCP_PORT_RANGE_1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ME2_TCP_PORT_RANGE_2 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ME3_TCP_PORT1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ME3_TCP_PORT2 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ME3_TCP_PORT_RANGE_1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ME3_TCP_PORT_RANGE_2 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $ME3_UDP_PORT_RANGE_1 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $MEA_TCP_PORT1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $MEA_TCP_PORT2 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $MEA_TCP_PORT3 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $MEA_TCP_PORT4 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $MEA_TCP_PORT_RANGE_1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $MEA_UDP_PORT_RANGE_1 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS -d $GGOGLE_TALK_SERVER_RANGE --dport $GOOGLE_TALK_UDP_PORT_RNG -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS -d $GGOGLE_TALK_SERVER_RANGE --dport $GOOGLE_TALK_UDP_PORT_RNG2 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $WOW_TCP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $WOW_TCP_PORT1 --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $WOW_TCP_PORT1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $WOW_TCP_PORT2 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $WOW_TCP_PORT3 --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $WOW_TCP_PORT4 -j ACCEPT
$IP6TABLES -A gaming_pc_in -p tcp --sport $UNPRIVPORTS --dport $WOW_TCP_PORT1 -j ACCEPT
$IP6TABLES -A gaming_pc_in -p udp --sport $UNPRIVPORTS --dport $WOW_TCP_PORT1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $WOW_TCP_CRASH_REPORT_PORT1 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $D4_SERVER_TCP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $D4_QUEUE_TCP_PORT_RNG1 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $FFXIV_TCP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $FFXIV_TCP_PORT_RNG2 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $FFXIV_TCP_PORT_RNG3 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $LARK_TCP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $LARK_TCP_PORT_RNG2 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $LARK_TCP_PORT_RNG3 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p tcp --sport $GW2_TCP_PORT1 --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $GW2_TCP_PORT2 --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $GW2_TCP_PORT2 -j ACCEPT
$IP6TABLES -A gaming_pc_in -p tcp --sport $UNPRIVPORTS --dport $GW2_TCP_PORT1 -j ACCEPT
$IP6TABLES -A gaming_pc_in -p tcp --sport $UNPRIVPORTS --dport $GW2_TCP_PORT2 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $RIFT_TCP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ML_TCP_PORT_RNG1 -j ACCEPT
...
TFD_UDP_PORT1="52848"
$IP6TABLES -A gaming_pc_out -p tcp --dport $TFD_TCP_PORT1 --sport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --dport $TFD_TCP_PORT2 --sport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --dport $TFD_UDP_PORT_RNG1 --sport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --dport $TFD_UDP_PORT1 --sport $UNPRIVPORTS -j ACCEPT
...
TL2_TCP_PORT1="4549"
TL2_UDP_PORT2="59243"
$IP6TABLES -A gaming_pc_out -p tcp --dport $TL2_TCP_PORT1 --sport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --dport $TL2_UDP_PORT2 --sport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A gaming_pc_in -p tcp --sport $UNPRIVPORTS --dport $TL2_TCP_PORT1 -j ACCEPT
$IP6TABLES -A gaming_pc_in -p udp --sport $UNPRIVPORTS --dport $TL2_UDP_PORT2 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $BITTORENT_TCP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $BITTORENT_TCP_PORT1 --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --sport $BITTORENT_UDP_PORT1 --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A gaming_pc_in -p udp --sport $UNPRIVPORTS --dport $BITTORENT_UDP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A gaming_pc_in -p tcp --sport $UNPRIVPORTS --dport $BITTORENT_TCP_PORT_RNG1 -j ACCEPT
$IP6TABLES -A gaming_pc_in -p tcp --sport $UNPRIVPORTS --dport $BITTORENT_TCP_PORT2 -j ACCEPT
...
STEAM_DED_SVR_UDP_PORT1="27015"
STEAM_DED_SVR_UDP_PORT2="27020"
$IP6TABLES -A gaming_pc_out -p udp --sport $STEAM_SVR_SEARCH_UDP_PORT2 --dport $UNPRIVPORTS -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $STEAM_FRIENDS_UDP_PORT -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $STEAM_UDP_PORT_RNG -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $STEAM_UDP_PORT_RNG2 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $STEAM_UDP_PORT_RNG3 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $STEAM_TCP_PORT_RNG -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $STEAM_TCP_PORT_FRIENDS_LIST -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $STEAM_TCP_CROSS_CONNECT_PORT -j ACCEPT
$IP6TABLES -A gaming_pc_in -p udp --sport $UNPRIVPORTS --dport $STEAM_DED_SVR_UDP_RNG -j ACCEPT
$IP6TABLES -A gaming_pc_in -p udp --sport $STEAM_UDP_SRC_PORT --dport $UNPRIVPORTS -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p udp --sport $GFW_LIVE_UDP_PORT --dport $GFWL_UDP_PORT -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $GFW_LIVE_UDP_PORT -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $GFWL_UDP_PORT3 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $OCULUS_UDP_PORT_RNG1 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $NEW_WORLD_UDP_PORT1 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $ROBLOX_UDP_PORT_RNG -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $ROBLOX_TCP_PORT1 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS --dport $NMS_UDP_STEAM_PORT -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --sport $UNPRIVPORTS -m multiport --dports $NMS_TCP_STEAM_PORT_LIST -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $NMS_UDP_STEAM_PORT_RNG1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $NMS_UDP_SERVER_PORT_RNG1 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p udp --sport $UNPRIVPORTS --dport $DDV_UDP_STEAM_PORT_RNG1 -j ACCEPT
...
$IP6TABLES -A gaming_pc_out -p tcp --dport $AOE_II_TCP_PORT -j ACCEPT
$IP6TABLES -A gaming_pc_out -p tcp --dport $AOE_II_TCP_PORT_RANGE_1 -j ACCEPT
$IP6TABLES -A gaming_pc_out -p udp --dport $AOE_II_UDP_PORT_RANGE_2 -j ACCEPT
$IP6TABLES -A gaming_pc_in -p tcp --dport $AOE_II_TCP_PORT -j ACCEPT
$IP6TABLES -A gaming_pc_in -p tcp --dport $AOE_II_TCP_PORT_RANGE_1 -j ACCEPT
$IP6TABLES -A gaming_pc_in -p udp --dport $AOE_II_UDP_PORT_RANGE_2 -j ACCEPT
...
#########################################################################################
$IP6TABLES -A log_tcp_state -p tcp -j LOG --log-prefix "(IPv6)(D)Illegal TCP state: " --log-ip-options --log-tcp-options
...
if [ "$USE_CONNECTION_TRACKING" = "1" ]; then
 if [ "$RUN_SILENTLY" != "1" ]; then
	...
	$IP6TABLES -A connection_tracking -m state --state ESTABLISHED,RELATED -j ACCEPT
...
$IP6TABLES -A source_address_check -s $INTERNET_IP -j LOG --log-prefix "(IPv6)(D)SrcAddrCk(ownIP): "
...
$IP6TABLES -A source_address_check -s $UNIQUE_LOCAL_ADDRESS -j LOG --log-prefix "(IPv6)(D)SrcAddrCk(UNIQUE_LOCAL_ADDRESS): " 
$IP6TABLES -A source_address_check -s $UNIQUE_LOCAL_ADDRESS -j DROP 
###########
## -cap comment out for setup testing
###########
$IP6TABLES -A source_address_check -s $LOOPBACK -j LOG --log-prefix "(IPv6)(D)SrcAddrCk(Loopback): " 
...
$IP6TABLES -A destination_address_check -p tcp -m multiport --destination-port $NFS_PORT,$SOCKS_PORT,$SQUID_PORT --syn -j LOG --log-prefix "(IPv6)(D)DstAddrCk(CmnPrtsTCP): " 
$IP6TABLES -A destination_address_check -p tcp -m multiport --destination-port $NFS_PORT,$SOCKS_PORT,$SQUID_PORT --syn -j DROP 
$IP6TABLES -A destination_address_check ! -s $LAN_SUBNET -p tcp --syn --destination-port $XWINDOW_PORTS -j LOG --log-prefix "(IPv6)(D)DstAddrCk(Xwindow-Ext): " 
$IP6TABLES -A destination_address_check ! -s $LAN_SUBNET -p tcp --syn --destination-port $XWINDOW_PORTS -j DROP 
# UDP unprivileged ports
# Deny connection requests to NFS and lockd ports
$IP6TABLES -A destination_address_check -p udp -m multiport --destination-port $NFS_PORT,$LOCKD_PORT -j LOG --log-prefix "(IPv6)(D)DstAddrCk(CmnPrtsUDP): " 
$IP6TABLES -A destination_address_check -p udp -m multiport --destination-port $NFS_PORT,$LOCKD_PORT -j DROP 
...
# ICMP rules #
##############
$IP6TABLES -A log_in -p icmpv6 ! --icmpv6-type echo-request -m limit -j LOG --log-prefix "(IPv6)(D)IN-drop: "
...
$IP6TABLES -A log_out -p icmpv6 --icmpv6-type destination-unreachable -j LOG --log-prefix "(IPv6)(D)OUT-icmp-dest-unrch-drop: "
$IP6TABLES -A log_out -p icmpv6 --icmpv6-type destination-unreachable -j DROP
# But log everything else
